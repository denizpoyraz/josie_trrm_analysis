...
for j in range(len(simlist)):
    ...
    I_s_ghv = smooth_gaussian(np.array(dft[j]['Tsim'].tolist()), np.array(dft[j]['IM'].tolist()), timew, sigma)
    Imib0_s_ghv = smooth_gaussian(np.array(dft[j]['Tsim'].tolist()), np.array(dft[j]['IminusiB0'].tolist()), timew, sigma)

    dft[j]['I_gsm'] = I_s_ghv
    dft[j]['IminiB0_gsm'] = Imib0_s_ghv
    ...
    # pre = 'fig2_'
    # plt.plot(dft[j].Tsim, dft[j].IM - dft[j].at[0,'iB0'], label = r'I$_M$ - I$_{B0}$', color =cbl[4])
    # plt.plot(dft[j].Tsim, dft[j].I_OPM_jma, label=r'I$_{OPM}$', color = cbl[0],linewidth=0.75)
    # plt.plot(dft[j].Tsim, dft[j].I_conv_slow_jma, label=r'I$_{OPM,C}$', color = cbl[2])
    # plt.plot(dft[j].Tsim, beta * dft[j].I_conv_slow_jma, label=r'S$_{S}\times$I$_{OPM,C}$', color = cbl[3])
    #
    ...
    plt.savefig('/home/poyraden/Analysis/JosieAnalysis/Plots/Plots_2023_final/v3/Plots_PerSim/png/upd2_' + pre + sp + '.png')
    plt.savefig('/home/poyraden/Analysis/JosieAnalysis/Plots/Plots_2023_final/v3/Plots_PerSim/eps/upd2_' + pre  + sp + '.eps')
    plt.savefig('/home/poyraden/Analysis/JosieAnalysis/Plots/Plots_2023_final/v3/Plots_PerSim/pdf/upd2_' + pre + sp + '.pdf')
    plt.show()
