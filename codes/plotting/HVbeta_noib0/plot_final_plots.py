...
for k in slist:
    ...
    plotname = f'{pre1}{pyear}_{labellist[k]}'
    ...
    if bool_rdif:
        ...
        if bool_three:
            plotname = f'v2_RDif_{pre1}{pyear}_{labellist[k]}_Scatter'
            ...
            plt.plot(profl[k]['RDif_cal'], profl[k]['Pair'], color=cbl[k], linestyle='None', marker='o',
                     label=f'TRC + Calibration', markersize=0.5)
            ax2.errorbar(rdif_IM_cal[k], Yp, xerr=rdif_IM_cal_err[k],
                         color='black', linewidth=3, elinewidth=1, capsize=1, capthick=1)

            ax2.set_yticklabels([])
            ax2.legend(loc='best', frameon=True, fontsize='xx-large', markerscale=10, handletextpad=0.1)

            # ax2.legend(loc='lower left', frameon=True, fontsize='xx-large', markerscale=10, handletextpad=0.1)
...
if bool_triple:
    ...
    for k in slist:
        ...
        ax.errorbar(rdif_IM_cal[k], Yp, xerr=rdif_IM_cal_err[k], label=f'TRC + Calibration',
