...
file_out = f'Josie{year}_deconv_2023paper_herman.csv'
# file_out = f'Josie{year}_deconv_2023paper{pre}.csv'

if bool_0910_decay:
    # file_out = f'Josie0910_deconv_2023_decay_added_147-149{pre}.csv'
    file_out = f'Josie0910_deconv_2023_decay_added_147-149{pre}_herman.csv'
...
for j in range(len(simlist)):
    ...
    if bool_sm_hv:
        ...
        for i in range(size - 1):
            ...
            Ifasth[i + 1] = dft[j].at[i + 1, 'IM_gsm']
            Ifasth[i] = dft[j].at[i, 'IM_gsm']
            Ifasth_deconv[i + 1] = (Ifasth[i + 1] - Ifasth[i] * Xf) / (1 - Xf)
    ...
    dft[j]['Ifasth'] = Ifasth
    dft[j]['Ifasth_deconv'] = Ifasth_deconv
