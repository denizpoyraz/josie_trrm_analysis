...
dfo = pd.read_csv("/home/poyraden/Analysis/JOSIEfiles/Proccessed/Josie0910_Deconv_preparationadded_simulation_2023paper_sm_hv_paper.csv")
...
df = df[(df.Sim == 145) & (df.Team == 3)]
dfo = dfo[(dfo.Sim == 145) & (dfo.Team == 3)]
# df = df[(df.Sim < 139)]
# dfo = dfo[(dfo.Sim < 139)]
...
for j in range(len(simlist)):
    ...
    dft[j].time_ib1 = dft[j].time_ib1/tconv
    dfto[j].time_ib1 = dfto[j].time_ib1/tconv
    ...
    plt.plot(dft[j].Tsim_new, dft[j]['IM-iB0'] , label=r'I$_M$ - I$_{B0}$', color = cbl[4] ) #blue
    ...
    plt.plot(dftib1.Tsim_new, dftib1['I_slow_conv_ib1_decay_all'],
             label=r'I$_{S}$ (I$_{B1}$-I$_{B0})\times X_S$]', linewidth='2.0', color = cbl[3], linestyle = '--')#green
