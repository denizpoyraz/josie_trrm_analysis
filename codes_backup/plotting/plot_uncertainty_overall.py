...
for i in ilist:
    ...
    if year == '0910':
        slist = ['PO3_dqa', 'PO3_OPM', 'PO3_trrm', 'PO3', 'IM','Ifast_minib0_deconv_sm10','I_slow_conv']
        clist = ['PO3_dqac', 'PO3_OPMc', 'PO3_trrmc','PO3',  'IMc','Ifast_minib0_deconv_sm10c','I_slow_convc']
        vlist = ['PFcor', 'iB0', 'iB1', 'iB2','Pair', 'ENSCI', 'Sol', 'Buf', 'Sim', 'Team',
                 'Tsim', 'TPext', 'Tpump_cor',  'Cpf_kom', 'Cpf_jma', 'unc_Cpf_kom', 'unc_Cpf_jma']
    ...
    ###plotting
    trrm_name = f'fig14_{pre}_{year}_TRRM_{labellist[i]}.png'
    con_name = f'fig_14{pre}_{year}_conventional_{labellist[i]}.png'
    print(trrm_name)
    print(con_name)
    size_label =20
    size_title =22
    ...
    plt.plot(df['d_eta_a'] * 100, df['Pair'], color=cbl[4], label=f'Absorption uncertainty ', linewidth=2)
