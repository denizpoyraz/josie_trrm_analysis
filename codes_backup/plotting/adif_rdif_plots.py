...
bool_noib0 = False
if bool_noib0: tag = '20percent_noib0_'
...
if year_0910:
    df = pd.read_csv(
        f"/home/poyraden/Analysis/JOSIEfiles/Proccessed/Josie0910_calibrated{pre}.csv", low_memory=False)
    print(list(df))

if year_9602 or year_2017:
    df['Ifast_minib0_deconv_sm10'] = df['Ifast_minib0_deconv'].rolling(window=5, center=True).mean()
    if bool_noib0:df['Ifast_minib0_deconv_sm10'] = df['Ifast_deconv'].rolling(window=5, center=True).mean()

    if bool_sm_vh:
        df['Ifast_minib0_deconv_sm10'] = df['Ifast_minib0_deconv']

        if bool_noib0:df['Ifast_minib0_deconv_sm10'] = df['Ifast_deconv']
...
if year_0910:
    ...
    if bool_noib0: df['Ifast_minib0_deconv_sm10'] = df['Ifast_deconv_ib1_decay']

    if bool_sm_vh:
        df['Ifast_minib0_deconv_sm10'] = df['Ifast_minib0_deconv_ib1_decay']
        if bool_noib0: df['Ifast_minib0_deconv_sm10'] = df['Ifast_deconv_ib1_decay']
        df['PO3_cor'] = (0.043085 * df['Tpump_cor'] * df['Ifast_minib0_deconv_sm10']) / (df['PFcor_jma'])
...
ptitle_trrm = f' {title}  TRC '
if bool_noib0: ptitle_trrm = f' {title}'+r' TRC (No I$_{B0}$ Correction)'
